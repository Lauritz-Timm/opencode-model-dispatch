...
describe("picker Svelte and OpenCode token contract", () => {
  ...
  test("picker uses themed OpenCode-style dropdowns instead of native selects", async () => {
    const app = await readText("picker/src/App.svelte")
    ...
    expect(app).toContain('ariaLabel="Configuration scope"')
    expect(app).toContain("onChange={setSetupScope}")
    ...
    expect(effortSelect.indexOf('{ value: "", label: "Auto" }')).toBeLessThan(effortSelect.indexOf("...options.map"))
    ...
    expect(compactSelect).toContain('role="option"')
    expect(compactSelect).toContain('aria-haspopup="listbox"')
    expect(compactSelect).toContain("aria-expanded={open}")
    expect(compactSelect).toContain("const listboxID =")
    expect(compactSelect).toContain("triggerButton?.focus()")
    expect(compactSelect).toContain("scrollIntoView")
    expect(compactSelect).toContain('block: "nearest"')
    expect(compactSelect).toContain("window.innerHeight")
    expect(compactSelect).toContain("visibleVerticalRange")
    expect(compactSelect).toContain("window.getComputedStyle(ancestor).overflowY")
    expect(compactSelect).toContain('overflowY === "auto"')
    expect(compactSelect).toContain("on:scroll|capture={handleGeometryChange}")
    expect(compactSelect).toContain("popoverMaxHeight")
    expect(compactSelect).toContain("overflow-y: auto")
    expect(compactSelect).not.toContain("overscroll-behavior: contain")
    expect(compactSelect).toContain("isPrintableTypeaheadKey")
    expect(compactSelect).toContain("typeaheadPrefix")
    expect(compactSelect).toContain('event.key === "ArrowDown"')
    expect(compactSelect).toContain('event.key === "Home"')
    expect(compactSelect).toContain('event.key === "End"')
    expect(compactSelect).toContain('event.key === "Enter"')
    expect(compactSelect).toContain('event.key === " "')
    expect(compactSelect).toContain('event.key === "Escape"')
    expect(compactSelect).toMatch(/\.compact-select\s*\{[^}]*height:\s*30px[^}]*box-sizing:\s*border-box/s)
    expect(compactSelect).toContain("background-color: var(--v2-background-bg-base)")
    expect(compactSelect).toContain("background: var(--v2-background-bg-layer-01)")
    expect(compactSelect).toContain("background: var(--v2-background-bg-layer-03)")
    expect(compactSelect).toContain("color: var(--v2-text-text-base)")
    expect(compactSelect).toContain("select-chevron")
    expect(compactSelect).toContain(".compact-select:focus-within")
    expect(compactSelect).toContain("var(--v2-border-border-focus)")
    expect(compactSelect).toContain(".compact-select.disabled")
