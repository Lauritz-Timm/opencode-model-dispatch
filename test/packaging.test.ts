...
describe("packaging and release assets", () => {
  ...
  test("public docs cover users and release operators", async () => {
    ...
    expect(releasing).toContain("use_immutable_subject=true")
    expect(releasing).toContain("Verify release-signing OIDC")
    expect(releasing).toContain("The final proof must come from a token minted")
    expect(releasing).not.toContain("sub_claim_prefix")
